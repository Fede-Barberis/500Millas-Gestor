...
export default function MateriaPrimaTable({ materiaPrimas, comprasMp, eliminarMateriaPrima, editarMateriaPrima }) {
    ...
    const confirmarEliminacion = async () => {
        await eliminarMateriaPrima(idAEliminar);
        setModalOpen(false);
        setIdAEliminar(null);
    };
    ...
    const filteredData = useMemo(() => {
        return rows.filter(item => {
            // Filtro por producto
            if (mpFiltro !== "all" && item.materiaPrima !== mpFiltro)
                return false;
    ...
    }, [rows, mpFiltro, fechaDesde, fechaHasta, isPagado, searchTerm]);

    // Columnas
    const columns = [
        columnHelper.accessor("id_compra", {
            header: "ID",
            cell: info => (
        ...
        columnHelper.accessor("materiaPrima", {
            header: "Insumo",
            cell: info => (
                <div className="flex items-center gap-2">
        ...
        columnHelper.accessor("acciones", {
            header: "Acciones",
            cell: info => {
                const compraCompleta = comprasMp.find(
                    c => c.id_compra === info.row.original.id_compra
                );
                ...
                return (
                    <div className="flex gap-2">
                        <button
                            type="button"
                            onClick={() => editarMateriaPrima(compraCompleta)}
                        ...
                        <button 
                            type="button"
                            onClick={() => abrirModal(info.row.original.id_compra)}
    ...
    return (
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
                    ...
                    <div>
                        ...
                        >
                            ...
                            {materiaPrimas.map(mp => (
                                <option key={mp.id_materiaPrima} value={mp.nombre}>
                                    {mp.nombre}
                                </option>
                            ))}
                        </select>
                ...
                {(mpFiltro !== "all" || fechaDesde || fechaHasta || isPagado !=="all" || searchTerm) && (
                    <button
                        onClick={() => {
                            setMpFiltro("all");
                            setFechaDesde("");
            ...
            <ConfirmModal
                ...
                message="¿Seguro que deseas eliminar esta compra? Esta acción devolverá el stock de materia prima."
            />
